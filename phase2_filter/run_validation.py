"""CLI for Phase-2 EKF validation ladder.
...
def _print_detect(res):
    ...
    print(f"  Peak SNR        : {res['accel_stats']['snr']:.3f}")
...
def _plot_result(res, step_name):
    ...
    times = np.array(result["times"]) / 86400.0
    ...
    ax.plot(times, da, label=r'$\Delta a$ [m]', alpha=0.8)
    ax.plot(times, dM * sm[:, 0], label=r'$\Delta M \cdot a$ [m]', alpha=0.8)
    ...
    ax.plot(times, innov[:, 0], label=r'$\nu_a$ [m]', alpha=0.6)
    ax.plot(times, innov[:, 5] * sm[:, 0], label=r'$\nu_M \cdot a$ [m]', alpha=0.6)
...
def main():
    parser = argparse.ArgumentParser(description="Phase-2 EKF validation ladder")
    parser.add_argument('--step', choices=['null', 'bias', 'impulse', 'detect', 'all'],
                        default='null')
    parser.add_argument('--duration-days', type=int, default=30)
    parser.add_argument('--dt-s', type=float, default=86400.0)
    parser.add_argument('--tau', type=float, default=None)
    parser.add_argument('--q', type=float, default=None)
    parser.add_argument('--rho-debris', type=float, default=1.0e-10)
    ...
    print(f"Phase-2 validation: step={args.step}  duration={args.duration_days}d  "
          f"tau={tau:.3e}  q={q:.3e}")

    steps_to_run = ['null', 'bias', 'impulse', 'detect'] if args.step == 'all' else [args.step]

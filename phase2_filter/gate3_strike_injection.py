#!/usr/bin/env python3
"""Gate 3: fleet-scale strike injection for detection efficiency ε(Δv).

Injects a single mid-duration strike across a Δv grid for each object in a
stratified fleet sample. Filter runs under the degraded "all" confounder
model. Reports ε(Δv), DV_50, DV_90.
...
def _propagate_truth_with_strike(
    ...
    """Propagate truth with a single strike injected at strike_time."""
    if Cd_base is None:
...
def _run_degraded_filter(
    ...
    """Forward+backward+smoother with flat F10.7, quiet Kp, fixed Cd."""
...
def run_strike_sweep_on_object(
    ...
    """Inject strikes at each Δv and record detection for one object."""
    try:
...
def main():
    ...
    print(f"Gate 3: threshold={args.threshold:.3f}  tau={args.tau:.3e}  q={args.q:.3e}")
    print(f"  Δv {args.dv_min:.2e} → {args.dv_max:.2e} ({args.n_dv} pts), "
          f"duration={args.duration_days:.0f}d")
    print()
    ...
    tasks = [
        (obj["tle_data"], args.duration_days, args.tau, args.q,
         dv_grid, args.threshold, args.seed_offset + idx)
        for idx, obj in enumerate(objects_to_process)
    ]
    ...
    if valid.any():
        ...
        print()
        if dv50 is not None:
            print(f"DV_50 = {dv50:.5f} m/s")
        else:
            print("DV_50: did not bracket 50%")
        if dv90 is not None:
            print(f"DV_90 = {dv90:.5f} m/s")
        else:
            print("DV_90: did not bracket 90%")
